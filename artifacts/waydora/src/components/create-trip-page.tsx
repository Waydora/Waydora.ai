...
export function CreateTripPage({ userId, trips, onSaveDraft, onPublish, onDelete, mobileOnly = false }: CreateTripPageProps) {
  ...
  const handlePublish = async () => {
    ...
    const savedResult = await onSaveDraft({
      ...
      description: draft.description, cover_photo: draft.coverPhoto,
      budget: draft.budget, best_season: draft.bestSeason, hero_emoji: draft.heroEmoji,
      status: "published", days: draft.days,
    });
    if (savedResult) {
  ...
  const previewItinerary = draft.destination ? {
    title: draft.title || "Anteprima", destination: draft.destination,
    durationDays: draft.days.length, vibe: draft.description,
    totalBudget: draft.budget, bestSeason: draft.bestSeason,
  ...
  // ── Editor content (condiviso desktop/mobile) ──────────────────────────
  const editorContent = (
        ...
        <div>
          ...
          {published.length > 0 && (
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "rgba(52,211,153,0.6)", marginBottom: "8px" }}>Pubblicati ({published.length}) — visibili in "Lasciati Ispirare"</div>
  ...
  // ── MOBILE: single column + modal mappa ───────────────────────────────
  if (mobileOnly) {
    return (
        ...
        {showMobileMap && (
          <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "#0a0a12", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px", borderBottom: "1px solid rgba(255,255,255,0.07)", flexShrink: 0, background: "rgba(10,10,18,0.95)" }}>
              ...
              <span style={{ fontSize: "15px", fontWeight: 700, color: "#fff" }}>Anteprima mappa</span>
            </div>
