  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "12px", marginBottom: "8px" }}>
      <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
        <input value={activity.time} onChange={(e) => onChange({ ...activity, time: e.target.value })} placeholder="09:00-11:00" style={{ ...inputStyle, width: "110px", flexShrink: 0 }} />
        <select value={activity.category} onChange={(e) => onChange({ ...activity, category: e.target.value })} style={{ ...inputStyle, flex: 1, cursor: "pointer" }}>
          {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.emoji} {c.label}</option>)}
        </select>
        <input value={activity.estimatedCost} onChange={(e) => onChange({ ...activity, estimatedCost: e.target.value })} placeholder="€15" style={{ ...inputStyle, width: "65px", flexShrink: 0 }} />
      ...
      <input value={activity.title} onChange={(e) => onChange({ ...activity, title: e.target.value })} placeholder="Nome del posto o attività" style={{ ...inputStyle, width: "100%", marginBottom: "6px", fontWeight: 600 }} />
      <textarea value={activity.description} onChange={(e) => onChange({ ...activity, description: e.target.value })} placeholder="Descrizione breve..." style={{ ...inputStyle, width: "100%", minHeight: "56px", resize: "vertical" }} />
    </div>
  );
}
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", marginBottom: "12px", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 14px", cursor: "pointer", borderBottom: collapsed ? "none" : "1px solid rgba(255,255,255,0.06)" }} onClick={() => setCollapsed(!collapsed)}>
        ...
        <input value={day.title} onChange={(e) => { e.stopPropagation(); onChange({ ...day, title: e.target.value }); }} onClick={(e) => e.stopPropagation()} placeholder={`Giorno ${dayIndex + 1}`} style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "#fff", fontSize: "13px", fontWeight: 700 }} />
        <button onClick={(e) => { e.stopPropagation(); onRemove(); }} style={{ width: "26px", height: "26px", borderRadius: "6px", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><X style={{ width: "12px", height: "12px" }} /></button>
        <div style={{ color: "rgba(255,255,255,0.4)" }}>{collapsed ? <ChevronDown style={{ width: "14px", height: "14px" }} /> : <ChevronUp style={{ width: "14px", height: "14px" }} />}</div>
      </div>
      {!collapsed && (
        <div style={{ padding: "12px 14px" }}>
          <input value={day.summary} onChange={(e) => onChange({ ...day, summary: e.target.value })} placeholder="Descrizione breve del giorno..." style={{ ...inputStyle, width: "100%", marginBottom: "12px" }} />
...
// ── TripListItem — mostra bozze e pubblicati in fondo all'editor ──────────
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", background: isDraft ? "rgba(255,255,255,0.04)" : "rgba(52,211,153,0.06)", border: isDraft ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(52,211,153,0.2)", borderRadius: "12px", marginBottom: "8px" }}>
      ...
      <div style={{ display: "flex", gap: "5px" }}>
        <button onClick={onEdit} title="Modifica" style={{ width: "28px", height: "28px", borderRadius: "7px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.5)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Edit3 style={{ width: "12px", height: "12px" }} />
        </button>
        <button onClick={onTogglePublish} title={isDraft ? "Pubblica" : "Torna a bozza"} style={{ width: "28px", height: "28px", borderRadius: "7px", background: isDraft ? "rgba(52,211,153,0.12)" : "rgba(255,255,255,0.06)", border: isDraft ? "1px solid rgba(52,211,153,0.25)" : "1px solid rgba(255,255,255,0.08)", color: isDraft ? "#34d399" : "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          {isDraft ? <Globe style={{ width: "12px", height: "12px" }} /> : <Eye style={{ width: "12px", height: "12px" }} />}
        </button>
        <button onClick={onDelete} title="Elimina" style={{ width: "28px", height: "28px", borderRadius: "7px", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
...
export function CreateTripPage({ userId, trips, onSaveDraft, onPublish, onDelete }: CreateTripPageProps) {
  ...
  // Aggiorna cover al cambio destinazione
  useEffect(() => {
    if (!draft.destination || draft.destination.length < 3) return;
    const timer = setTimeout(() => {
      setCoverLoading(true);
      fetchPhoto(`${draft.destination} landmark travel`).then((url) => setDraft(d => ({ ...d, coverPhoto: url }))).catch(() => {}).finally(() => setCoverLoading(false));
    }, 800);
    return () => clearTimeout(timer);
  }, [draft.destination]);

  // Carica una bozza/pubblicato nell'editor
  const loadTrip = (trip: UserTripRow) => {
    setDraft({
      id: trip.id, title: trip.title, destination: trip.destination,
      description: trip.description, days: trip.days ?? [newDay(0)],
      coverPhoto: trip.cover_photo || FALLBACK, budget: trip.budget,
      bestSeason: trip.best_season, heroEmoji: trip.hero_emoji, status: trip.status,
    });
    setSaved(false);
  };

  const toDays = () => draft.days;

  const handleSaveDraft = async () => {
    if (!draft.title || !userId) return;
    setSaving(true);
    const result = await onSaveDraft({
      id: draft.id, title: draft.title, destination: draft.destination,
      description: draft.description, cover_photo: draft.coverPhoto,
      budget: draft.budget, best_season: draft.bestSeason, hero_emoji: draft.heroEmoji,
      status: "draft", days: toDays(),
    });
  ...
  const handlePublish = async () => {
    ...
    setPublishing(true);
    // Prima salva
    const savedResult = await onSaveDraft({
      id: draft.id, title: draft.title, destination: draft.destination,
      description: draft.description, cover_photo: draft.coverPhoto,
      budget: draft.budget, best_season: draft.bestSeason, hero_emoji: draft.heroEmoji,
      status: "published", days: toDays(),
    });
    if (savedResult) {
      await onPublish(savedResult.id);
      setDraft(d => ({ ...d, id: savedResult.id, status: "published" }));
    }
    setPublishing(false);
  };
  ...
  const handleCoverUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setDraft(d => ({ ...d, coverPhoto: URL.createObjectURL(file) }));
    e.target.value = "";
  };

  // Preview mappa
  const previewItinerary = draft.destination ? {
    title: draft.title || "Anteprima",
    destination: draft.destination, durationDays: draft.days.length,
    ...
    days: draft.days.map((d, i) => ({
      day: i + 1, title: d.title, summary: d.summary,
      activities: d.activities.filter(a => a.title).map(a => ({ ...a })),
    })),
    packingList: [],
  } : null;
  ...
  return (
    <div style={{ height: "100%", display: "flex", overflow: "hidden", background: "#0a0a12" }}>
      ...
      {/* ── Editor ── */}
      <div style={{ flex: "0 0 50%", overflowY: "auto", borderRight: "1px solid rgba(255,255,255,0.07)", padding: "20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "16px" }}>
          <div style={{ display: "flex", gap: "8px" }}>
            <input value={draft.heroEmoji} onChange={(e) => setDraft(d => ({ ...d, heroEmoji: e.target.value }))} placeholder="🗺️" style={{ ...inputStyle, width: "48px", textAlign: "center", fontSize: "18px" }} />
            <input value={draft.title} onChange={(e) => setDraft(d => ({ ...d, title: e.target.value }))} placeholder="Titolo del viaggio *" style={{ ...inputStyle, flex: 1 }} />
          </div>
          <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
            <MapPin style={{ width: "14px", height: "14px", color: "rgba(255,255,255,0.35)", flexShrink: 0 }} />
            <input value={draft.destination} onChange={(e) => setDraft(d => ({ ...d, destination: e.target.value }))} placeholder="Destinazione * (es. Tokyo, Giappone)" style={{ ...inputStyle, flex: 1 }} />
          </div>
          <textarea value={draft.description} onChange={(e) => setDraft(d => ({ ...d, description: e.target.value }))} placeholder="Descrizione breve..." style={{ ...inputStyle, minHeight: "64px", resize: "vertical" }} />
          <div style={{ display: "flex", gap: "8px" }}>
            <input value={draft.budget} onChange={(e) => setDraft(d => ({ ...d, budget: e.target.value }))} placeholder="💰 Budget (es. €800)" style={{ ...inputStyle, flex: 1 }} />
            <input value={draft.bestSeason} onChange={(e) => setDraft(d => ({ ...d, bestSeason: e.target.value }))} placeholder="🌤 Periodo" style={{ ...inputStyle, flex: 1 }} />
          </div>
        </div>
        ...
        <div style={{ marginBottom: "14px" }}>
          ...
          <button onClick={() => setDraft(d => ({ ...d, days: [...d.days, newDay(d.days.length)] }))}
            style={{ width: "100%", padding: "8px", borderRadius: "10px", background: "rgba(255,255,255,0.04)", border: "1px dashed rgba(255,255,255,0.14)", color: "rgba(255,255,255,0.45)", fontSize: "12px", fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
          ...
          <div>
            ...
            {drafts.length > 0 && (
              <div style={{ marginBottom: "16px" }}>
                <div style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "rgba(255,255,255,0.35)", marginBottom: "8px" }}>
                  Bozze ({drafts.length})
                </div>
                {drafts.map(t => (
                  <TripListItem key={t.id} trip={t}
                    ...
                    onTogglePublish={() => onPublish(t.id)} />
                ))}
              </div>
            )}
            ...
            {published.length > 0 && (
              <div style={{ marginBottom: "24px" }}>
                <div style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "rgba(52,211,153,0.6)", marginBottom: "8px" }}>
                  Pubblicati ({published.length}) — visibili in "Lasciati Ispirare"
                </div>
                {published.map(t => (
                  <TripListItem key={t.id} trip={t}
                    onEdit={() => loadTrip(t)}
                    ...
                    onTogglePublish={async () => {
                      // Torna a bozza
                      await onSaveDraft({ id: t.id, title: t.title, status: "draft" });
                    }} />
                ))}
              </div>
            )}
