...
function DayHeader({ destination, dayIndex, title, weather, summary }: {
  destination: string; dayIndex: number; title: string; weather?: string; summary: string;
}) {
  const photoSrc = usePhoto(`${destination} ${title}`);
  return (
    <>
      <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-md py-4 mb-5 -mx-4 md:mx-0 px-4 md:px-0 border-b border-border/50">
        <div className="flex items-baseline justify-between gap-3 flex-wrap">
          <div className="flex items-baseline gap-3">
            ...
            <span className="text-muted-foreground font-medium text-sm md:text-base">{title}</span>
          </div>
          {weather && (
        ...
        <p className="text-sm text-muted-foreground mt-2 italic border-l-2 border-accent pl-3">{summary}</p>
      </div>
      <div
...
function HeroPhoto({ destination }: { destination: string }) {
  const photoSrc = usePhoto(destination);
  return (
    <div
    ...
    <div className="space-y-10">
      <div className="rounded-3xl overflow-hidden border border-border bg-card shadow-xl">
        <div className="relative">
          <HeroPhoto destination={itinerary.destination} />
        ...
        <div className="p-5 md:p-6 flex flex-wrap gap-2">
          <Badge variant="outline" className="px-3 py-1 bg-background text-foreground border-border/60">
            <MapPin className="w-3 h-3 mr-1.5 text-accent" />{itinerary.destination}
          </Badge>
          <Badge variant="outline" className="px-3 py-1 bg-background text-foreground border-border/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {list.map((category: any, catIndex: number) => (
            <div key={category.category} className="space-y-3">
              <h4 className="font-bold text-xs uppercase tracking-wider text-accent">{category.category}</h4>
