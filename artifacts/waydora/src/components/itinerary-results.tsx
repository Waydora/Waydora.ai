...
// Suggerisce una query Amazon sensata per ogni articolo del bagaglio
function getAmazonQuery(item: string, category: string): string {
  const q = item.toLowerCase();
  // Mappatura intelligente per articoli comuni
  ...
  if (q.includes("mascherina") || q.includes("maschera")) return "mascherina viaggio confortevole";
  ...
  if (q.includes("sim") || q.includes("esim")) return "sim internazionale viaggio";
  if (q.includes("asciugamano")) return "asciugamano microfibra viaggio compatto";
  // Fallback: usa il nome dell'articolo + viaggio
  return `${item} viaggio`;
}

// Categorie del bagaglio che hanno senso su Amazon
const AMAZON_CATEGORIES = [
  "Abbigliamento", "Accessori", "Tecnologia", "Salute e Igiene",
  "Essenziali", "Documenti", "Sport e Outdoor",
];

// ── Palette categoria attività ────────────────────────────────────────────
...
function TripPhoto({ query, isHero }: { query: string; isHero: boolean }) {
  ...
  useState(() => {
    fetchPhoto(query)
      .then((url) => { setSrc(url); setLoaded(true); })
      .catch(() => { setSrc(FALLBACK); setLoaded(true); });
  });
...
function TripPhotos({ queries, destination }: { queries?: string[]; destination: string }) {
  const photoQueries = (queries && queries.length > 0)
    ? queries.slice(0, 4)
    : [`${destination} landmark`, `${destination} street`, `${destination} food`];
  ...
  return (
    <div style={{ padding: "16px", overflowY: "auto", height: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        ...
        {/* Badge Amazon */}
        <a href={`https://www.amazon.it/s?k=accessori+viaggio&tag=${AMAZON_TAG}`} target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "10px", fontWeight: 700, padding: "4px 10px", borderRadius: "9999px", background: "rgba(255,153,0,0.15)", color: "#ff9900", border: "1px solid rgba(255,153,0,0.3)", textDecoration: "none" }}>
          ...
          return (
            <div key={category.category}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <h4 style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.2em", color: "rgba(255,255,255,0.5)" }}>
                  {category.category}
                </h4>
                {hasAmazonLink && (
                  <a href={amazonLink(`${category.category} viaggio`)} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize: "10px", color: "#ff9900", textDecoration: "none", opacity: 0.7 }}
              ...
              <ul style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                {category.items.map((item: string, itemIndex: number) => {
                  ...
                  const amazonQuery = getAmazonQuery(item, category.category);
                  const link = amazonLink(amazonQuery);
                  ...
                  return (
                    <li key={itemIndex}
                      ...
                      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "6px" }}>
                        <span style={{
                          color: isChecked ? "rgba(255,255,255,0.28)" : "rgba(255,255,255,0.75)",
                          textDecoration: isChecked ? "line-through" : "none", lineHeight: 1.4,
                        }}>
                          {item}
                        </span>
                        {/* Icona Amazon inline */}
                        <a href={link} target="_blank" rel="noopener noreferrer"
                          title={`Cerca "${item}" su Amazon`}
                          style={{ flexShrink: 0, opacity: 0.45, transition: "opacity 0.15s", color: "#ff9900", textDecoration: "none" }}
                          onMouseEnter={(e) => (e.currentTarget.style.opacity = "1")}
      ...
      <div style={{ marginTop: "20px", paddingTop: "16px", borderTop: "1px solid rgba(255,255,255,0.07)", textAlign: "center" }}>
        <a href={`https://www.amazon.it/s?k=accessori+viaggio${destination ? `+${encodeURIComponent(destination)}` : ""}&tag=${AMAZON_TAG}`}
          ...
          Tutto il necessario per il viaggio su Amazon
        </a>
